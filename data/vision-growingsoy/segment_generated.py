...
EXPERIMENT = "generated-weeds-paid"
...
def summarize(run):
    ...
    cost = (tokens["promptTokenCount"] * run["pricing_usd_per_million"]["input"]
            + output_tokens * run["pricing_usd_per_million"]["output"]) / 1_000_000
    run["summary"] = {
        "counts": dict(counts),
...
def save_report(directory, run):
    ...
    lines = [
        f"# Malezas en {len(run['results'])} imagenes generadas — {run['model']}",
        "",
    ...
    lines += [
        ...
        "",
        f"![Resumen de las {len(run['results'])} vistas](contact-sheet.jpg)",
        "",
    ]
...
def run(billing_acknowledged, ids, model):
    ...
    if model not in PRICING_USD_PER_MILLION:
        raise SystemExit(f"Unsupported model for priced pilot: {model}")
    inputs = load_inputs(ids)
    experiment = EXPERIMENT + "-" + model.replace(".", "-") + "-" + "-".join(point_id.lower() for point_id in ids)
    ...
    run_data = {
        ...
        "pricing_usd_per_million": PRICING_USD_PER_MILLION[model],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", default=MODEL, choices=tuple(PRICING_USD_PER_MILLION))
    args = parser.parse_args()
    check() if args.check else run(args.billing_acknowledged, tuple(args.ids.split(",")), args.model)
